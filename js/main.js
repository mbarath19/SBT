...
//   const apiUrl = "http://localhost:5000"; // Make sure this is the correct API URL
  ...
  function renderProducts(products) {
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="product-item position-relative bg-light overflow-hidden h-100 shadow-sm">
                        ...
                        <div class="text-center p-4">
                            ...
                            <!-- Product Description -->
                            <p class="mb-2 p-2 text-muted">${
                              product.description || "No description available"
                            }</p>
                            
                            <!-- Product Price -->
                            <span class="text-primary fw-bold">Rs. ${
                              product.price
                            }</span>
                        </div>
    ...
    <small class="w-100 text-center py-2">
        <button class="text-body w-100 btn" ${
          product.soldOut === "on" ? "disabled" : ""
        } data-product-id="${
        product.id
  ...
  // Global Add to Cart button click handler
            ...
            <div class="col-lg-3 col-md-6">
                <div class="product-item d-flex flex-column position-relative bg-light overflow-hidden h-100 shadow-sm">
                    ...
                    <div class="image-container position-relative">
                        <img class=" d-block  mx-auto" src="${product.imageUrl}" alt="${product.name}">
                        ${product.soldOut === "on" ? `
                    ...
                    <div class="text-center py-2 flex-grow-1">
                        ...
                        <p class="text-muted mb-1 p-2" style="margin-right: 5px; word-wrap: break-word;">
                    ...
                    <div class="mt-auto">
                        ...
                        <div class="d-flex border-top">
                            <small class="w-100 text-center py-2">
                                <button class="text-body w-100 btn add-to-cart-btn"
                                    ${product.soldOut === "on" ? "disabled" : ""}
                                    data-product-id="${product.id}">
                                    <i class="fa fa-shopping-bag me-2"></i>
                                    ${product.soldOut === "on" ? "Sold Out" : "Add to Cart"}
                                </button>
                            </small>
                        </div>
                    </div>
...
// `;
  ...
  function createCategoryButtons(categories) {
    ...
    // Default "All Products" button
    const allProductsButton = `
            <li class="nav-item me-2">
                <a class="btn category-button active" href="javascript:void(0);" onclick="filterByCategory('')" id="allProductsButton">
                    All Products
                </a>
            </li>
        `;
    ...
    categories.forEach((category) => {
      const button = `
                <li class="nav-item me-2">
                    <a class="btn category-button" href="javascript:void(0);" onclick="filterByCategory('${category._id}')" id="category-${category._id}">
                        ${category.name}
                    </a>
                </li>
            `;
